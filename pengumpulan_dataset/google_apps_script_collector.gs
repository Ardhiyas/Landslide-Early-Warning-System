...
function doGet(e) {
  ...
  } else if (sheetName === "LoRaTest") {
    if (sheet.getLastRow() === 0) {
      sheet.appendRow([
        "Waktu", "Data Terkirim", "Data Diterima", "RSSI (dBm)", "Delay (s)", "Keterangan"
      ]);
    }
    sheet.appendRow([
      timestamp, p.dataTerkirim, p.dataDiterima, p.rssi, p.delay, p.keterangan
    ]);
